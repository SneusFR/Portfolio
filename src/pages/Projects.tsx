    ...
    const interval = setInterval(() => {
      setCurrentImageIndexes(prev => {
        ...
        filteredProjects.forEach(project => {
          const images = getCarouselImages(project.name);
          const currentIndex = newIndexes[project.name] || 0;
          newIndexes[project.name] = (currentIndex + 1) % images.length;
        });
        return newIndexes;
      });
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-16">
            {filteredProjects.map((project, index) => {
              ...
              const currentImageIndex = currentImageIndexes[project.name] || 0;
              
              return (
                <div key={`${project.name}-${index}`} className="group relative">
                  {/* Carte glassmorphing avec hauteur fixe */}
                  <div
                    role="button"
                    tabIndex={0}
                    ...
                    className={`glass-project-card relative p-6 cursor-pointer h-[620px] flex flex-col backdrop-blur-xl border shadow-2xl transition-all duration-500 hover:scale-105 rounded-3xl ${
                      isDarkMode
                        ? 'bg-gray-900/40 border-gray-700/30'
                        : 'bg-white/40 border-white/30'
                    }`}
                  >
                    {/* Carrousel d'images avec hauteur optimisée */}
                    <div className="relative mb-4 w-full h-56 rounded-2xl overflow-hidden bg-gray-100 dark:bg-gray-800">
                      <img
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span
                        className="category-badge text-xs font-semibold px-2 py-1 rounded-full backdrop-blur-sm border transition-all duration-200"
                        style={{
                    ...
                    <div className="flex flex-wrap gap-1 mb-4">
                      {project.technologies.slice(0, 3).map(tech => (
                        <span
                          key={tech}
                          className={`tech-tag px-2 py-1 text-xs font-semibold rounded-full backdrop-blur-sm border transition-all duration-200 ${
                            isDarkMode
                      ...
                      {project.technologies.length > 3 && (
                        <span
                          className={`tech-tag px-2 py-1 text-xs font-semibold rounded-full backdrop-blur-sm border ${
                            isDarkMode
                              ? 'bg-blue-500/15 border-blue-500/35 text-blue-300'
                              : 'bg-blue-500/15 border-blue-500/35 text-blue-700'
                          }`}
                        >
                          +{project.technologies.length - 3}
                        </span>
                    ...
                    <div className="flex gap-3 mt-auto">
                      <a
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded-full font-medium transition-all duration-300 hover:scale-105 text-sm ${
                          isDarkMode
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-2 rounded-full font-medium transition-all duration-300 hover:scale-105 bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:shadow-lg hover:shadow-blue-500/25 cursor-pointer text-sm"
